...
public class MonJoueur implements IJoueur {
    ...
    public String choixMouvement() {
        ...
        if (!initialDone) {
            ...
            // On inscrit ces pièces sur le plateau
        ...
        // --- Phase de mouvement ---
        // 1) Vérifier si on a AU MOINS une pièce de lisère = lastLisere
        ...
        for (int r=0; r<6; r++) {
            for (int c=0; c<6; c++) {
                if (plateau[r][c] == couleur) {
                    // appliquer la contrainte uniquement si effectiveLisere>0
                    if (effectiveLisere > 0 && LISERE[r][c] != effectiveLisere) {
                        continue;
                    }
                    int max = LISERE[r][c];
        ...
        if (legal.isEmpty()) {
            ...
            // on choisit un coup aléatoire
            String coup = legal.get(new Random().nextInt(legal.size()));
            // MAJ du plateau et de la lisère pour rester synchro
            String[] p = coup.split("-");
    ...
    public void mouvementEnnemi(String coup) {
        ...
        // placement initial de l’adversaire (slash)
        if (coup.contains("/")) {
            int advCol = -couleur;
